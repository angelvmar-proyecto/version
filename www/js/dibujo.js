...
function dibujarTodo() {
  ...
  if (window.lineasVisibles) {
    ...
    // 🟢 ML (minimo local)
    if (window.lineasMLV && window.lineasMLV.length > 0) {
      ctx.strokeStyle = CONFIG.COLOR_ML || '#84cc16';
      ctx.lineWidth = grosor;
      window.lineasMLV.forEach(function(x) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, fuente.height);
        ctx.stroke();
    ...
    // 🟠 WS (whitespace)
    if (window.lineasWSV && window.lineasWSV.length > 0) {
      ctx.strokeStyle = CONFIG.COLOR_WS || '#f97316';
      ctx.lineWidth = grosor;
      window.lineasWSV.forEach(function(x) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, fuente.height);
        ctx.stroke();
